...
**活跃变量分析**：对进行代码优化有重要作用。当我们在寄存器中计算一个值后，通常假设在某个块中还要引用它，如果它在该块的末尾是不活跃的，那么它在程序的后续过程中不再出现，即变得无用，那么我们不再需要存储该值；消除复制四元式的依据也是对活跃变量的分析，如果某个变量的值在以后不被引用，那么该复制四元式可以被消除。
...
#### 实现的类与其功能
- LvaRunner
作为程序运行的UI交互类，负责接收用户指令并执行指令，输出结果/CFG图

- LvaMain
活跃变量分析的启动类，负责配置soot分析时的运行参数并调用Soot执行

- AnalysisTransformer
活跃变量分析的转换器类，负责将相关的方法生成CFG图并调用活跃变量分析方法，输出相应的分析结果
